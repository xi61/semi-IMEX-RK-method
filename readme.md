...
| `semIIMEXRungeKutta.m` | Index for the same semi-IMEX method |
...
The MATLAB code has been translated to Python in this repository. The folder mapping is:
...
|------|-------------------|---------|
| `semi_imex.py` | `SemiIMEXRungeKutta.m` | Main configurable semi-IMEX solver: `semiIMEXRungeKutta()`, `semi_imex_trapezoid()`, `semi_imex_rk2()` |
| `imex.py` | `IMEXRungeKutta2/3.m` | IMEX RK2/RK3 for $y' = f_{\text{ex}} + g_{\text{im}}$ |
| `explicit.py` | — | RK4 explicit solver, used as a reference |
| `step.py` | `RKOneStep.m` | `ButcherTableau` dataclass and single-step function |
| `problems.py` | — | Predefined test problems and error metrics |
| `utils.py` | *(no equivalent)* | Added to handle Python-specific issues (see below) |
...
| `[2, 3, 4]` | 2nd-order, 3-stage, index 4 (Table 5) |
| `[2, 3, 3]` | 2nd-order, 3-stage, index 3 (Table 7) |
...
| `test_scalar_ode_convergence.py` | `paper_scalar_equation.m` | Reproduces **Table 11**: convergence tests for all 2nd- and 3rd-order methods on the scalar ODE $y' = \cos(t)\,y + (-y + \cos(t))\,y$ |
...
Example Usage to reproduce Table 11, see:
